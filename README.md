...
| `AI_BRAND_NAME`, `AI_OWNER_NAME`, `AI_OWNER_ALIAS`, `AI_OWNER_ROLE`, `AI_OWNER_BIO`, `AI_COMPANY_NAME`, `AI_COMPANY_URL` | — | **Moved to code** → `src/config/identity.js` |
| `SOCIAL_TIKTOK`, `SOCIAL_INSTAGRAM`, `SOCIAL_YOUTUBE`, `SOCIAL_LINKEDIN`, `SOCIAL_GITHUB` | — | **Moved to code** → `src/config/identity.js` |
...
> 💡 **Crypto prices & social stats need no env vars.** They are **static** and live in code — edit [`src/config/crypto.js`](src/config/crypto.js) and [`src/config/social.js`](src/config/social.js), then restart the server. This means a visitor can never discover an API key, because there isn't one.
